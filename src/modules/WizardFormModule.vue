...
const { currentStep, handleNext, handlePrev, steps, maxAgeError } =
  useStep(age);
...
<template>
  <WizardWrapper>
    <template #stepper>
      <StepperWizard class="mb-5" :steps="steps" v-if="!maxAgeError" />
    </template>
    <template #content>
      <div v-if="!maxAgeError">
    ...
    <template #error>
      <section class="text-center" v-if="maxAgeError">
        <ErrorView />
        <ButtonMain class="w-[200px] mr-10" @click="maxAgeError = false">
          Ok :(
        </ButtonMain>
      </section>
    </template>
